...
### Configuração Atual de CORS (Dinamizada)

Agora as origens são gerenciadas via `Settings` e podem ser customizadas no `.env.local` do backend.

**Arquivo:** `services/api/core/config.py`
**Arquivo de Env:** `services/api/.env.local`

Para adicionar uma nova origem (ex: seu IP atual), adicione no `.env.local`:
```bash
ALLOW_ORIGINS=["http://localhost:3000", "http://127.0.0.1:3000", "http://172.19.91.58:3000"]
```

**Vantagens:**
- Não precisa alterar o código fonte para adicionar novos IPs
- Suporta múltiplos ambientes (WSL, Docker, Local Network)
- O middleware CORS injeta automaticamente variantes comuns de localhost

#### IP Atual da Máquina
O seu IP atual no WSL é `172.19.91.58`. Se estiver acessando de outro lugar, certifique-se que este IP (ou o IP correspondente) esteja na lista `ALLOW_ORIGINS`.
