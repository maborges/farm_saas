...
class ClimaService(BaseService[RegistroClima]):
    ...
    async def sync_open_meteo(self, fazenda_id: UUID) -> str:
    ...
    async def get_clima_periodo(self, fazenda_id: UUID, data_inicio: date, data_fim: date):
    ...
    async def get_chuva_acumulada(self, fazenda_id: UUID, dias: int = 30) -> float:
        hoje = date.today()
